...
if (isset($_GET["id"])) {
  $id = $_GET["id"];
  $sql = "SELECT * FROM durable_material_repair as r,durable_material as m WHERE r.id = $id";
...
<body id="page-top">
  ...
  <div id="wrapper">
    ...
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-md-6 offset-md-3">
          <div class="card shado mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-danger body-text"><i class="fas fa-wrench"></i> รับคืนการซ่อมวัสดุคงทน</h6>
            </div>
            <div class="card-body">
                      ...
                      <div class="row">
                        <div class="col-10 ">
                          <select class="form-control body-text" name="damage_id" id="damage_id" value="<?php echo $item["damage_id"]; ?>">
                            <?php
                          $sqlSelectType = "SELECT *,d.id as dmid FROM durable_material_damage as d,durable_material as a WHERE a.id = d.product_id and d.status = 1";
                          $resultType = mysqli_query($conn, $sqlSelectType);
                           while ($row = mysqli_fetch_assoc($resultType)) {
                             if ($item["damage_id"] == $row["dmid"]) {
                               echo '<option value="' . $row["dmid"] . '"selected>' . $row["code"] . " : " . $row["flag"] .  '</option>';
                             } else {
                               echo '<option value="' . $row["dmid"] . '">' . $row["code"] . " : " . $row["flag"] .  '</option>';
                             }
                           }
                            ?>
                          </select>
                ...
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-group body-text">
                      ...
                      <textarea class="form-control body-text" name="place" id="place" placeholder="" rows="3"><?php echo $item["place"]; ?></textarea>
        ...
        <div class="row">
            <div class="col-6 offset-3 card" style="padding: 10px" align="center">
  ...
  <script>
    ...
    function sendData() {
      ...
      // params["data"] = data;
      $.ajax({
        url: 'service/service_edit_durable_material_repair_history.php?id=<?php echo $_GET["id"]; ?>',
        type: 'POST',
        dataType: 'JSON',
        data: {
          body: data
        ...
        success: function(data) {
          console.log(data);
          if (data.result) {
            window.location = "display_durable_material_repair.php";
          }
        },
        error: function(error) {
          console.log(error);
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        คุณต้องการแก้ไขข้อมูลรายการซ่อมวัสดุหรือไม่ ?
      </div>
      <div class="modal-footer">
...
<div class="modal fade" id="exampleModal1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        คุณต้องการเพิ่มข้อมูลรายการซ่อมวัสดุหรือไม่ ?
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">ยกเลิก</button>
