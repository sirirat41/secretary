...
if (isset($_GET["id"])) {
  $id = $_GET["id"];
  $sql = "SELECT * FROM durable_material_sell WHERE id = $id";
  $result = mysqli_query($conn, $sql) or die('cannot select data');
  $item = mysqli_fetch_assoc($result);
  $selldate = $item["sell_date"];
  $newselldate = date("ํY-m-d", strtotime($selldate));
  $show=10;
}
?>
<!DOCTYPE html>
        ...
        <div class="col-md-6 offset-md-3">
          <div class="card shado mb-6">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-danger"><i class="fas fa-hand-holding-usd"></i> แก้ไขข้อมูลการขายทอดตลาด(วัสดุคงทน)</h6>
            </div>
            <div class="card-body">
