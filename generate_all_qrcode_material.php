...
if (isset($_GET["type"]) && isset($_GET["page"]) && $_GET["type"] == "fix") {
    ...
    if (strpos($page, "-") == true) {
        ...
        $start = $exp[0] -1;
        $count = $exp[1] - $start;
        $sql = "SELECT * FROM durable_material Order by id LIMIT $start, $count";
        ...
        for ($i = 0; $i < sizeOf($exp); $i++) {
            if ($sql != "") {
                $sql .= " union (SELECT * FROM durable_material LIMIT " . ($exp[$i] - 1) . " , 1)";
            } else {
                $sql .= "(SELECT * FROM durable_material LIMIT " . ($exp[$i] - 1) . " , 1)";
            }
        }
    } else {
        $page -= 1; // $page = $page - 1;
        $sql = "SELECT * FROM durable_material Order by id LIMIT $page, 1";
    }
} else {
    $sql = "SELECT * FROM durable_material Order by id";
}
    ...
    <div class="modal fade" id="modal-config" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="generate_all_qrcode_material.php" method="GET">
