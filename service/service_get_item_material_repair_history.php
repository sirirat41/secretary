...
if (isset($_POST['id'])) {
  $damage_id = $_POST["id"];
  $sqlSelectProductid = "SELECT product_id FROM durable_material_damage WHERE id = $damage_id";
  ...
  $sql = "SELECT * FROM durable_material_damage WHERE product_id = $productid";
  $resultdamage = mysqli_query($conn, $sql);
  $response = array();
//  echo $sql;
  while ($damage = mysqli_fetch_assoc($resultdamage)) {

    $sqlrepair = "SELECT * FROM durable_material_repair WHERE damage_id = " . $damage["id"];
